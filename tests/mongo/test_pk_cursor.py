...
def test_pk_cursor_dedup():
    ...
    assert type(r) == mongo_pk_reader
    t = db()
    assert type(t) == mongo_pk_cursor
    d = dictable(a = [1,2,3])*dict(b = [1,2,3])
    t.raw.drop()
    t = t.insert_many(d)
    t.raw.insert_many(d(_pk = [['a', 'b']])) ## creating duplicates
    t.raw.insert_many(d(_pk = [['a', 'b']])) ## creating duplicates
    assert len(t) == 27
...
def test_pk_cursor_raw():
    t = mongo_table('test', 'test', pk = ['a', 'b'])
    assert isinstance(t.raw, mongo_cursor) and t.raw.spec == {}
    r = t.raw.find(a = 1)
    assert isinstance(r.raw, mongo_cursor) and r.raw.spec == {}
